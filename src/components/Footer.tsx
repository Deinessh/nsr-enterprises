...
export const Footer: React.FC = () => {
  ...
  return (
    <footer className="bg-gradient-to-b from-[#E0F2FE] via-[#DCEEFE] to-[#BAE6FD] text-brand-dark pt-16 pb-20 border-t-2 border-[#93C5FD] shadow-inner relative overflow-hidden">
      
      {/* Subtle Ambient Decorative Circles */}
      <div className="absolute top-0 right-10 w-80 h-80 bg-white/40 rounded-full blur-3xl pointer-events-none" />
      <div className="absolute bottom-0 left-10 w-96 h-96 bg-sky-200/50 rounded-full blur-3xl pointer-events-none" />

      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-12 gap-10 pb-12 border-b border-sky-300/60">
          
          {/* Col 1: Globe Logo & Text Brand Logo Side-by-Side */}
          <div className="lg:col-span-4 space-y-4">
            <Link to="/" className="inline-flex items-center space-x-3 p-3 rounded-2xl bg-white/80 backdrop-blur-md border border-sky-200 shadow-md hover:shadow-lg transition-all">
              <img 
                src="/logo-icon.png" 
                alt="NSR Globe Logo" 
            ...
            <div className="flex items-center space-x-3 pt-1">
              <a
                ...
                className="w-10 h-10 rounded-xl bg-white border border-sky-300 text-brand-blue hover:bg-brand-blue hover:text-white flex items-center justify-center shadow-sm transition-all hover:scale-105"
              ...
              <div className="inline-flex items-center space-x-1.5 px-3 py-1.5 rounded-xl bg-white border border-sky-300 text-[11px] font-extrabold text-brand-green shadow-sm">
                <ShieldCheck className="w-3.5 h-3.5 text-brand-green" />
                <span>Certified Bio Engineering</span>
              </div>
            </div>
          </div>
          ...
          <div className="lg:col-span-2 space-y-4">
            <h4 className="font-display font-extrabold text-base sm:text-lg text-brand-dark tracking-tight border-b border-sky-300/60 pb-2">
              Navigation
            </h4>
          ...
          <div className="lg:col-span-3 space-y-4">
            ...
            <ul className="space-y-2.5 text-sm font-bold text-slate-700">
              {SERVICES.map((s) => (
                <li key={s.id}>
                  <Link to={`/services/${s.id}`} className="hover:text-brand-blue transition-colors flex items-center space-x-2">
        ...
        {/* Bottom Bar */}
        <div className="pt-8 flex flex-col sm:flex-row justify-between items-center text-xs text-slate-600 space-y-4 sm:space-y-0 font-bold">
          <p>© {new Date().getFullYear()} NSR Enterprises. All Rights Reserved. Bio Septic Tanks & STP Solutions Tamil Nadu.</p>
          <button
            onClick={scrollToTop}
            className="flex items-center space-x-2 text-brand-dark font-extrabold hover:text-brand-blue transition-colors bg-white px-4 py-2 rounded-xl border border-sky-300 shadow-sm hover:shadow-md"
          >
